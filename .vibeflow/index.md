...
7. **Catalog write endpoints have no role/ownership check** (`POST /v1/catalog/{artists,albums,tracks}` require only `RequireAuth`, i.e. any authenticated user can write shared catalog data) — flagged as a documented TODO in the README (deviation #5) but still open. Low severity while there's no real content-ingestion pipeline, but worth a spec before any public signup opens.
